// Author: Jose Garcia
// Program: lab8 Coding Exam
// Purpose: Calculates your Body Shape Index
//CMPS 335
//Exam framework.
...
int main(int argc, char *argv[])
{
    ...
    #define TEST_MODE
    #ifdef TEST_MODE
    Flt rows = sizeof(testdata)/sizeof(testdata[0]);
    printf("%lf rows!???\n",rows);
    printf("Unit test for SBSI function...\n");
    printf("tolerance: %s\n", argv[1]);
    printf("H    WC   BSA    VTC  SBSI       Function\n");
    printf("---  ---  -----  ---  ---------  ---------\n");
    Flt value;
    Flt diff;
    int i;
    for (i = 0; i < 81; i++)
        ...
        sbsi.bsa = testdata[i*5+3];
        sbsi.vtc = testdata[i*5+2];
        value = bodyShapeIndex(&sbsi);
        diff = value - testdata[i*5+4];
        diff = fabs(diff);
        if (diff > 0.005)
            printf("%lf %lf %lf %lf %lf %lf<---Outside of tolerance\n",
            sbsi.h,sbsi.wc,sbsi.bsa,sbsi.vtc,
            value,diff);
        else
            printf("%lf %lf %lf %lf %lf %lf\n",sbsi.h,sbsi.wc,sbsi.bsa,
            sbsi.vtc,value,diff);
        return 0;
    #endif
	//
